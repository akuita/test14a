...
import { EntitySchema, Not, DataSource, ObjectType, FindOptionsWhere, IsNull } from 'typeorm'
...
export class EntityUniqueValidator implements ValidatorConstraintInterface {
  ...
  async isNonDuplicateCheckIn(employee_id: number, date: string): Promise<boolean> {
    const attendanceRepo = this.dataSource.getRepository(AttendanceRecord)
